...
public class NewDomIterator extends DomIterator
{
	
	NewDomIterator(ITabHandler t)
	{
		super(t);
	}
	...
	public boolean next() 
	{
		...
		do
		{
			...
			if(prevRelativeNode != null && !((Element)prevRelativeNode.getParentNode()).getAttribute("style").contains("display:none") && !((Element)prevRelativeNode.getParentNode()).getAttribute("style").contains("display: none"))
			{
